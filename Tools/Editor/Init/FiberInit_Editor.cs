...
public class FiberInit_Editor : AInvokeHandler<FiberInit, ETTask>
{
    ...
    private static void TestScene(Scene currentScene)
    {
        ...
        cube.AddComponent<TransformComponent, Vector3, Quaternion, Vector3>(new Vector3(0, 5, 0), new Vector3(45, 0, 45).ToQuaternion(), Vector3.One);
        ...
        AudioComponent audioComponent = player.AddComponent<AudioComponent>();
        audioComponent.PlayImmediately(new SignalGenerator()
        {
            Gain = 0.2f,
            Frequency = 500,
