...
public static class AnalyzerHelper
{
    ...
    public static SyntaxNode? GetFirstChild(this SyntaxNode syntaxNode)
    {
        ...
        var syntaxNodes = childNodes as SyntaxNode[] ?? childNodes.ToArray();
        if (syntaxNodes.Any())
        {
            return syntaxNodes.First();
        }
    ...
    public static bool HasAttributeInTypeAndBaseTypes(this ITypeSymbol typeSymbol, string AttributeName)
    {
    ...
    /// 获取最近的指定类型祖先节点
    /// </summary>
    public static T? GetNearestAncestor<T>(this SyntaxNode syntaxNode) where T:SyntaxNode
    {
            
        foreach (var ancestorNode in syntaxNode.Ancestors())
        {
            if (ancestorNode is T ancestor)
            {
                return ancestor;
            }
        }
    ...
    /// 判断函数是否是否含有指定类型的参数
    /// </summary>
    public static bool HasParameterType(this IMethodSymbol methodSymbol, string parameterType, out IParameterSymbol? cancelTokenSymbol)
    {
        foreach (var parameterSymbol in methodSymbol.Parameters)
        {
            if (parameterSymbol.Type.ToString() == parameterType)
            {
                cancelTokenSymbol = parameterSymbol;
                return true;
            }
        }
        cancelTokenSymbol = null;
        return false;
    }
    ...
    public static BasicBlock? GetAwaitStatementControlFlowBlock(StatementSyntax statementSyntax,AwaitExpressionSyntax awaitExpressionSyntax ,SemanticModel semanticModel)
    {
        ...
        var methodSyntax = statementSyntax.GetNearestAncestor<MethodDeclarationSyntax>();
    ...
    public static bool IsETEntity(this ITypeSymbol typeSymbol)
    {
        ...
        return typeName == Definition.EntityType || baseType == Definition.EntityType;
    }

    /// <summary>
    /// 类型symbol是否是EntityRef 或EntityWeakRef类
    /// </summary>
