...
public class EventSystem: Singleton<EventSystem>, ISingletonAwake
{
    ...
    {
        if (!this.allEvents.TryGetValue(typeof(T), out var iEvents))
        {
            return;
        }
        ...
        {
            if (!IsSame(scene.SceneType, eventInfo.SceneType))
            {
                Log.Instance.Error($"event sceneType error: {eventInfo.IEvent.GetType().FullName}, {scene.SceneType}, {eventInfo.SceneType}");
                continue;
            }
                    
            if (!(eventInfo.IEvent is AEvent<S, T> aEvent))
            {
                Log.Instance.Error($"event error: {eventInfo.IEvent.GetType().FullName}");
                continue;
            }
